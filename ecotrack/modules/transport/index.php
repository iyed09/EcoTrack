...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $stmt = $pdo->prepare("DELETE FROM transport_entries WHERE id = ? AND user_id = ?");
    $stmt->execute([$id, $userId]);
    redirect('index.php');
}

$stmt = $pdo->prepare("SELECT te.*, tt.name as transport_name, tt.emission_per_km FROM transport_entries te JOIN transport_types tt ON te.transport_id = tt.id WHERE te.user_id = ? ORDER BY te.date DESC");
$stmt->execute([$userId]);
...
foreach ($entries as $entry) {
    $totalEmissions += $entry['distance_km'] * $entry['emission_per_km'];
